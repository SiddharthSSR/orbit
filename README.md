...
Current project health and the recommended next MVP block are recorded in [`docs/checkpoints/mvp-3-32-project-health.md`](docs/checkpoints/mvp-3-32-project-health.md). The completed Project Intelligence arc (project-linked memories/todos, project detail, and optional project-scoped Ask) and its guardrails are documented in [`docs/checkpoints/mvp-4-9-project-intelligence.md`](docs/checkpoints/mvp-4-9-project-intelligence.md). The completed Daily Operating Surface arc (the Today Project Digest with due cues, caught-up state, and a header summary) and its guardrails are documented in [`docs/checkpoints/mvp-6-5-daily-operating-surface.md`](docs/checkpoints/mvp-6-5-daily-operating-surface.md). The completed Capture / Inbox Intelligence arc (capture quality metadata, the needs-review cue, Inbox filters, and a read-only memory detail view) and its guardrails are documented in [`docs/checkpoints/mvp-7-3-capture-inbox-intelligence.md`](docs/checkpoints/mvp-7-3-capture-inbox-intelligence.md).
...
The Today, Ask, Inbox, Bills, and Projects screens already adopt them. Inbox/Memory leans
...
That address lets the simulator reach the FastAPI server running on your Mac. The Today tab combines live Todos, Bills, Memory, and the latest Mood check-in into a dashboard. It also lets you submit a simple Mood check-in. The Bills tab uses the live Bill API to load, create, mark paid/unpaid, and delete bills. The Inbox tab uses the live Memory API to load, capture, archive, and delete memory items. The Projects tab uses the live Project API to load, create, update status, archive, filter, and delete projects. Project detail shows linked todos, linked memories, and a read-only activity summary (linked/open/completed counts, next due todo, and last linked memory date) computed from that already-loaded data. Today todo rows include a project menu to assign, change, or unlink a todo's project (unlink sends an explicit null), and Project detail refreshes its linked todos when a link changes. The linked project name is shown consistently across Today, Inbox, and Project detail rows via a shared `LinkedProjectLabel`. The Ask tab uses the backend `/ask` API with the deterministic mock AI provider and includes a lightweight context inspection panel for previewing backend context. Ask can optionally be scoped to one project (opt-in): when a project is selected, the memory context is limited to that project's linked memories via an optional `project_id` on the Ask request; with no project selected, Ask behaves exactly as before. The context preview honors the same scope and labels itself when project-scoped. Existing clients use keyword retrieval by default; hybrid retrieval is backend-only and opt-in until the iOS client adopts it.
